Version 1.0.2 (2026-03-22)
==========================
* LaneMode / Laneless 전환 안내
  * LFA 버튼으로 LaneMode와 Laneless를 전환할 때 화면에 MID 안내 문구가 표시되도록 추가
...
==========================
* Onroad UI 조정
  * LaneMode에서는 현재 주행 차선만 초록색으로 강조하고 path는 흰색으로 정리
  * Laneless에서는 차선은 흰색, path는 초록색으로 구분되도록 조정
  * 흰색 차선 두께를 키워 저속/야간 화면에서도 더 잘 보이도록 보강
* 방향지시등 / 차선 변경 표시
  * 실제 차량 깜빡이 상태에 맞춰 속도 좌우에 방향지시등 아이콘이 점멸하도록 변경
  * 차선 변경 시 이벤트 문구를 유지하면서, 현재 차선과 목표 차선의 바깥 경계만 초록색으로 강조하고 가운데 차선은 숨기도록 정리
  * 차선 변경 안내음은 제거해 주행 중 불필요한 알림을 줄임
* 빌드 / 배포 안정화
  * comma 기기 첫 빌드에서 번역 `.ts` 파일을 직접 갱신하지 않도록 조정해 `git pull` 이후 tracked 파일이 더러워지는 문제를 줄임
